...
Pipelines are containers — runners never call them directly. Instead, they recurse into the pipeline's steps. The `StepMeta::is_leaf()` method returns `false` for pipelines, signaling the runner to descend into children.
