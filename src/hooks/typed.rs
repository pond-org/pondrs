...
pub trait TypedHook<T: 'static>: Sync {
    fn after_load(&self, _n: &dyn StepMeta, _ds: &DatasetRef, _value: &T) -> Result<(), HookAbort> { Ok(()) }
    fn before_save(&self, _n: &dyn StepMeta, _ds: &DatasetRef, _value: &T) -> Result<HookControl, HookAbort> { Ok(HookControl::Continue) }
}

pub struct TypedHookAdapter<T, H>(H, PhantomData<fn() -> T>);

impl<T: 'static, H: TypedHook<T> + Sync> Hook for TypedHookAdapter<T, H> {
    fn after_dataset_loaded(&self, n: &dyn StepMeta, ds: &DatasetRef, value: &dyn core::any::Any) -> Result<(), HookAbort> {
    ...
    fn before_dataset_saved(&self, n: &dyn StepMeta, ds: &DatasetRef, value: &dyn core::any::Any) -> Result<HookControl, HookAbort> {
        if let Some(v) = value.downcast_ref::<T>() {
